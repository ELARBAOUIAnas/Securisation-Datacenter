...
Zone2::Zone2(QWidget *parent) : QWidget(parent)
{
    setWindowTitle("Zone 2");
    resize(600, 400);
    ...
    connect(historiqueAlertesButton, &QPushButton::clicked, this, &Zone2::showHistoriqueAlertes);
    connect(historiqueAccesButton, &QPushButton::clicked, this, &Zone2::showHistoriqueAcces);
    connect(listeEmployesButton, &QPushButton::clicked, this, &Zone2::showListeEmployes);
    connect(retourButton, &QPushButton::clicked, this, &Zone2::retour);

    // Timer pour rafraîchir les alertes toutes les 5 secondes
    refreshAlertesTimer = new QTimer(this);
    connect(refreshAlertesTimer, &QTimer::timeout, this, &Zone2::updateHistoriqueAlertes);
    ...
    connect(refreshAccesTimer, &QTimer::timeout, this, &Zone2::updateHistoriqueAcces);
    refreshAccesTimer->start(1000); // Rafraîchit toutes les 5 secondes
}

void Zone2::setupDatabase()
{
...
void Zone2::showHistoriqueAlertes()
{
    setupDatabase();
    ...
    query.prepare("SELECT date_alerte, description FROM alerte_zone2 ORDER BY date_alerte DESC");
...
void Zone2::updateHistoriqueAlertes()
{
    if (!isVisible()) return;
...
void Zone2::showHistoriqueAcces()
{
    setupDatabase();
    ...
    query.prepare("SELECT date_acces, description FROM acces_zone2 ORDER BY date_acces DESC");
...
void Zone2::updateHistoriqueAcces()
{
    if (!isVisible()) return;
...
void Zone2::showListeEmployes()
{
    ...
    QSqlQuery query;
    query.prepare("SELECT nom, prenom FROM employes_zone2 ORDER BY nom ASC");
...
void Zone2::updateListeEmployes()
{
