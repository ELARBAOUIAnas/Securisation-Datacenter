...
Zone1::Zone1(QWidget *parent) : QWidget(parent)
{
    setWindowTitle("Zone 1");
    resize(600, 700);
    ...
    connect(historiqueAlertesButton, &QPushButton::clicked, this, &Zone1::showHistoriqueAlertes);
    connect(historiqueAccesButton, &QPushButton::clicked, this, &Zone1::showHistoriqueAcces);
    connect(listeEmployesButton, &QPushButton::clicked, this, &Zone1::showListeEmployes);
    connect(employesBloquesButton, &QPushButton::clicked, this, &Zone1::showEmployesBloques);
    connect(retourButton, &QPushButton::clicked, this, &Zone1::retour);

    // Timers pour rafraîchir les historiques (actualisation toutes les 5 secondes)
    refreshAlertesTimer = new QTimer(this);
    connect(refreshAlertesTimer, &QTimer::timeout, this, &Zone1::updateHistoriqueAlertes);
    refreshAlertesTimer->start(5000);

    refreshAccesTimer = new QTimer(this);
    connect(refreshAccesTimer, &QTimer::timeout, this, &Zone1::updateHistoriqueAcces);
...
// ------------------------
void Zone1::showHistoriqueAlertes()
{
    ...
    QString sqlStr = "SELECT date_alerte, description FROM alerte_zone1 ORDER BY date_alerte DESC";
...
void Zone1::updateHistoriqueAlertes()
{
    if (!isVisible()) return;
    // On réutilise la même requête que showHistoriqueAlertes
    QSqlDatabase db = QSqlDatabase::database();
    QSqlQuery query(db);
    query.prepare("SELECT date_alerte, description FROM alerte_zone1 ORDER BY date_alerte DESC");
    if (!query.exec())
        return;
...
// Affichage des accès
// ------------------------
void Zone1::showHistoriqueAcces()
{
    setupDatabase();
    QSqlDatabase db = QSqlDatabase::database();
    ...
    query.prepare("SELECT date_acces, description FROM acces_zone1 ORDER BY date_acces DESC");
    if (!query.exec()) {
        QMessageBox::critical(this, "Erreur", "Échec de la requête : " + query.lastError().text());
        return;
    }
...
void Zone1::updateHistoriqueAcces()
{
    if (!isVisible()) return;
...
void Zone1::showListeEmployes()
{
    ...
    QSqlQuery query(db);
    query.prepare("SELECT nom, prenom FROM employes_zone1 ORDER BY nom ASC");
    if (!query.exec()) {
        QMessageBox::critical(this, "Erreur", "Échec de la requête : " + query.lastError().text());
        return;
    }
...
void Zone1::updateListeEmployes()
{
    if (!isVisible()) return;
    QSqlQuery query;
    query.prepare("SELECT nom, prenom FROM employes_zone1 ORDER BY nom ASC");
...
void Zone1::showEmployesBloques()
{
    refreshAccesTimer->stop();
    refreshAlertesTimer->stop();
    setupDatabase();
    ...
    QSqlQuery query;
    query.prepare("SELECT UID, nom, prenom, bloque FROM RFID_Zone1 WHERE bloque = 1");
    ...
    tableWidget->clear();
    tableWidget->setColumnCount(4);
    tableWidget->setHorizontalHeaderLabels({"UID", "Nom", "Prénom", "Action"});
    tableWidget->setRowCount(0);
    ...
    while (query.next()) {
        ...
        connect(btnDebloquer, &QPushButton::clicked, this, &Zone1::debloquerEmployeFromButton);
...
void Zone1::debloquerEmployeFromButton()
{
    ...
    QSqlDatabase db = QSqlDatabase::database();
    QSqlQuery query(db);
    query.prepare("UPDATE RFID_Zone1 SET tentatives_echouees = 0, bloque = 0 WHERE nom = :uid");
...
Zone1::Zone1(QWidget *parent) : QWidget(parent)
{
    ...
    employesBloquesButton->setStyleSheet(employesBloquesButton->styleSheet() +
                                         "background-color: darkblue; border-color: darkblue;");
    ...
    buttonVerticalLayout->addWidget(employesBloquesButton);
